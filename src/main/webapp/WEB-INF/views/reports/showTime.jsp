        ...
        <table id="report_list">
            <tbody>
                <tr>
                    ...
                    <th class="report_staratTime">出勤時間</th>
                    <th class="report_endTime">退勤時間</th>
                    <th class="report_action">操作</th>
                </tr>
                <c:forEach var="report" items="${reports}" varStatus="status">
                    <fmt:parseDate value="${report.reportDate}" pattern="yyyy-MM-dd"
                        var="reportDay" type="date" />
                    <fmt:parseDate value="${report.startTime}" pattern="HH:mm" var="startTime" type="time" />
                    <fmt:parseDate value="${report.endTime}" pattern="HH:mm" var="endTime" type="time" />
                    ...
                    <tr class="row${status.count % 2}">
                        ...
                        <td class="report_startTime"><fmt:formatDate value='${startTime}' pattern='HH時mm分' /></td>
                        <td class="report_endTime"><fmt:formatDate value='${endTime}' pattern='HH時mm分' /></td>
